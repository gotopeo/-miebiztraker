...
- [x] 開発環境でスクレイピングをテスト（長時間実行のため本番環境でテスト）して動作確認
...
## 実行中ステータスのまま残っているログを解除して再発防止

### 問題
手動スクレイピングの実行中ステータスが3つ残っている（2026/2/22 19:00:41、18:43:38、2026/2/20 18:00:00）。これらは実際には実行されていない可能性が高い。

### 原因
- サーバーの再起動やクラッシュにより、スクレイピング処理が中断されたが、ステータスが「実行中」のまま残っている
- タイムアウト検知機能がないため、長時間実行中のままになっているログを自動的に失敗に更新できない

### フェーズ1: 実行中ステータスのログを特定して失敗に更新
- [x] データベースで実行中ステータスのログを確認
- [x] 開始時刻から10分経過したログを「失敗」に更新
- [x] エラーメッセージを「Process terminated or timeout (automatically cleaned up)」に設定

### フェーズ2: 再発防止策を実装（タイムアウト検知機能）
- [x] cleanupJob.tsにタイムアウトしたスクレイピングログのクリーンアップ機能を追加
- [x] 10分以上「実行中」のままのログを「失敗」に自動更新
- [x] 毎日03:00 JSTに定期実行（既存のスケジューラーを利用）

### フェーズ3: チェックポイントを保存してデプロイ
- [x] チェックポイントを保存
- [ ] Publishボタンでデプロイ
- [ ] 本番環境で手動スクレイピングをテストし、実行中ステータスが正常に更新されることを確認


---

## 管理者画面（/admin）をプレビューのトップページに設定

### 要件
- プレビュー時やデプロイ後、トップページ（/）にアクセスすると管理者画面（/admin）が表示されるようにする
- 一般ユーザー用の画面（現在の/）は別のパス（例: /public）に移動

### フェーズ1: ルーティングを修正して/adminをトップページに設定
- [x] App.tsxのルーティングを確認
- [x] /を管理者画面（Home）に変更
...
### フェーズ3: 開発環境でテスト
- [x] 開発環境でスクレイピングをテスト（長時間実行のため本番環境でテスト）実行
- [ ] エラーがないことを確認
...
## スクレイパーの取得件数制限を300件に拡張

### 問題
現在のスクレイパーは10件しか取得できていない。全件取得するには300件の制限が必要。

### フェーズ2: 取得件数を300件に拡張する修正を実装
- [ ] ページネーション制限を300件に拡張
- [ ] または、全ページを取得するように修正

### フェーズ3: テストを実行して全件取得を確認
- [x] 開発環境でスクレイピングをテスト（長時間実行のため本番環境でテスト）
- [ ] 300件以上取得できることを確認（本番環境でテスト予定）

### フェーズ4: チェックポイントを作成してユーザーに報告
- [ ] チェックポイントを保存
- [ ] Publishボタンでデプロイ
- [ ] 本番環境でテスト
### フェーズ1: スクレイパーのコードを調査して取得件数制限の原因を特定
- [x] scraper.tsのコードを確認
- [x] ページネーション処理を確認
- [x] 取得件数の制限箇所を特定

### フェーズ2: 取得件数を300件に拡張する修正を実装
- [x] ページネーション制限を300件に拡張
- [x] または、全ページを取得するように修正



---

## スクレイパーが10件しか取得できない問題を修正

### 問題
本来100件以上あるはずなのに、10件しか取得できていない。ページネーション処理が正しく動作していない可能性がある。

### フェーズ1: スクレイパーのログとコードを詳細に調査して10件制限の原因を特定
- [ ] 過去のスクレイピングログを確認
- [ ] ページネーション処理のコードを詳細に確認
- [ ] 「次へ」ボタンの検出とクリック処理を検証

### フェーズ2: ページネーション処理の動作を確認し、次ページへの遷移が正しく行われているか検証
- [ ] 実際のWebサイトで「次へ」ボタンの存在を確認
- [ ] 「次へ」ボタンのセレクタが正しいか確認
- [ ] ページ遷移後の待機時間が適切か確認

### フェーズ3: 問題を修正して100件以上取得できるように実装
- [ ] ページネーション処理を修正
- [ ] より詳細なログを追加

### フェーズ4: テストを実行して100件以上取得できることを確認
- [ ] 開発環境でスクレイピングをテスト
- [ ] 100件以上取得できることを確認
...
## 通知設定に「区分」（工事・委託）フィルターを追加

- [ ] shared/constructionTypes.ts に工事・委託の分類定義を追加
- [ ] DBスキーマ（subscriptions）に category フィールドを追加してマイグレーション
- [ ] 管理者用通知設定ページ（NotificationSettings.tsx）に区分選択UIと工種連動を実装
- [ ] LIFF用通知設定ページ（LiffNotificationSettings.tsx）に同様の実装
- [ ] 通知フィルタリングロジック（notifier.ts等）に区分フィルターを適用
- [ ] チェックポイントを保存

## updatedAt誤更新による重複通知の修正

- [x] newBiddingDetector.tsのlastSeenAt更新時にupdatedAtを明示的に保持する
- [x] チェックポイントを保存
